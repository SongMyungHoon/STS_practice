...
public class StudentDao {
		...
		try {
			if(connection != null) {
				...
				while(resultSet.next()) {
					...
					student = new ExamRIO(resultSet.getString("name")
	...
	public List<ExamRIO> selectAll() {
		...
		List<ExamRIO> studentList = new ArrayList<>();
		try {
			if(connection != null) {
				statement = connection.createStatement();
				/* executeQuery: SELECT문만 실행 가능
				...
				while(resultSet.next()) {
					studentList.add(new ExamRIO(resultSet.getString("name")
												, resultSet.getInt("studentId")
												, resultSet.getInt("kor")
												, resultSet.getInt("eng")
												, resultSet.getInt("mat")));
	...
	public String insert(ExamRIO student) {
		dbConnect(); // database Connection 생성
